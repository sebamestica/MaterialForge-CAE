...
export const useLabStore = create<LabState>((set, get) => ({
  ...
  runDynamicSimulation: () => {
    ...
    const predictedMaxForce = get().predictions?.maxForceNewtons || (get().material.toUpperCase() === "TPU" ? 1200.0 : 8000.0);
    const maxForce = Math.ceil(predictedMaxForce * 1.25);

    set({ isSimulating: true, appliedForce: 0 });

    let currentForce = 0;
    const step = Math.max(10, Math.ceil(maxForce / 50));
