...
import { getFailureState } from "@/rendering/pipelines/tpms/StructuralDamageSystem";
  ...
  return (
    <div className="w-full h-full bg-white flex flex-col select-none text-slate-800 overflow-hidden">
      ...
      <div className="p-3 space-y-4 flex-1 overflow-y-auto scrollbar-thin">
        ...
        {/* SECTION: STRUCTURAL FAILURE VISUALIZATION (SFVS) */}
        <SFVSPanel />
...
function MechanicalCharts({ material, infill }: { material: string; infill: number }) {
  ...
  const { maxStress, failureStrain, elasticLimit, utsStrain } = useMemo(() => {
    if (matUpper === "TPU") {
      return {
        elasticLimit: 22,
        failureStrain: 48,
        utsStrain: 48,
        maxStress: yieldStress * 2.5,
      };
      ...
      return {
        ...
        failureStrain: 38,
        utsStrain: 38,
        maxStress: yieldStress * 1.5,
      };
    }
  }, [matUpper, yieldStress]);
  ...
  const utsStress = maxStress;
  const maxForceX = (utsStrain / 50) * maxDeform;
  ...
  const chartData = useMemo(() => {
    ...
    if (graphTab === "esfuerzo") {
      for (let i = 0; i <= 50; i += 2) {
        if (i <= elasticLimit) {
          ...
          const softeningRate = matUpper === "PLA" ? 0.45 : matUpper === "TPU" ? 0.05 : 0.2;
          const stress = maxStress * Math.exp(-(i - failureStrain) * softeningRate);
      ...
      for (let i = 0; i <= 50; i += 2) {
        ...
        let force = 0;
        if (i <= elasticLimit) {
          force = (i / elasticLimit) * (maxForceVal * 0.7);
        } else if (i <= failureStrain) {
          const t = (i - elasticLimit) / (failureStrain - elasticLimit);
          force = (maxForceVal * 0.7) + (maxForceVal * 0.3) * Math.sin((t * Math.PI) / 2);
        } else {
          const dropRate = matUpper === "PLA" ? 0.3 : matUpper === "TPU" ? 0.02 : 0.15;
          force = maxForceVal * Math.exp(-(i - failureStrain) * dropRate);
        }
      ...
      for (let i = 0; i <= 50; i += 2) {
        ...
        let energy = 0;
        if (matUpper === "TPU") {
          energy = maxEnergy * Math.pow(i / 50, 1.8);
        } else {
    ...
    return data;
  }, [graphTab, matUpper, yieldStress, maxStress, failureStrain, elasticLimit, maxForceVal, maxDeform, maxEnergy]);

  const activePoint = useMemo(() => {
    if (!chartData || chartData.length === 0) return null;
    const predictedMaxForce = predictions?.maxForceNewtons || (material.toUpperCase() === "TPU" ? 1200.0 : 8000.0);
    const maxForceSlider = Math.ceil(predictedMaxForce * 1.25);
    const fraction = appliedForce / maxForceSlider;
    const index = Math.min(
      chartData.length - 1,
      Math.max(0, Math.floor(fraction * (chartData.length - 1)))
    );
    return chartData[index];
  }, [chartData, appliedForce, predictions, material]);
...
function SFVSPanel() {
  const { appliedForce, predictions, material } = useLabStore(
    useShallow((state) => ({
      appliedForce: state.appliedForce,
      predictions: state.predictions,
      material: state.material,
  ...
  const predictedMaxForce = predictions?.maxForceNewtons || (material.toUpperCase() === "TPU" ? 1200.0 : 8000.0);
  const damageFactor = Math.min(1.5, Math.max(0.0, appliedForce / predictedMaxForce));
  const activeState = getFailureState(damageFactor);

  return (
    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-xs font-sans">
      <div className="bg-slate-50/70 px-3 py-2 border-b border-slate-200 flex justify-between items-center">
        <span className="text-sm text-slate-500 font-bold uppercase tracking-wider block font-extrabold">
          VISUALIZACIÓN DE FALLA ESTRUCTURAL
        </span>
        <span className="text-[10px] text-slate-400 font-mono">Estado: Activo</span>
      </div>
      <div className="p-3 space-y-3">
        <div className="flex justify-between items-center">
          <span className="text-sm text-slate-500 font-bold">Estado actual:</span>
          <span className={`text-xs font-black uppercase px-2 py-0.5 rounded border ${activeState.colorClass}`}>
            {activeState.label}
          </span>
        </div>

        <p className="text-[11px] text-slate-500 font-medium leading-relaxed bg-slate-50/50 p-2 rounded border border-slate-100">
          {activeState.description}
        </p>

        <div className="space-y-1">
          <div className="flex justify-between text-xs font-semibold text-slate-500">
            <span>Factor de daño (damageFactor):</span>
            <span className="font-bold text-slate-800 font-mono">{(damageFactor).toFixed(3)}</span>
          </div>
          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all duration-300 ${
                damageFactor >= 1.15 ? "bg-red-600" : damageFactor >= 0.90 ? "bg-orange-500" : "bg-emerald-500"
              }`}
              style={{ width: `${(damageFactor / 1.5) * 100}%` }}
            />
          </div>
        </div>

        <div className="text-[10px] text-slate-400 font-medium leading-normal bg-amber-50/30 p-2.5 rounded border border-amber-100/50">
          ⚠️ <strong>Visualización estimada basada en métricas estructurales. No corresponde a una simulación FEA de fractura.</strong>
        </div>
      </div>
    </div>
  );
