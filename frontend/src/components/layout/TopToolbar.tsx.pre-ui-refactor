...
import { exportStl } from "@/cad/exporters/stl";
import { exportGcode } from "@/cad/exporters/gcode";
import { exportPdf } from "@/cad/exporters/pdf";
...
export default function TopToolbar() {
  const store = useLabStore(
    useShallow((state) => ({
      ...
      loadProjectConfig: state.loadProjectConfig,
    ...
    if (file.name.endsWith(".json")) {
      ...
      reader.onload = (event) => {
        try {
          ...
          store.loadProjectConfig(config);
        } catch (err) {
      ...
      try {
        ...
        const res = await fetch(`${BACKEND_URL}/api/import_project`, {
          method: "POST",
          body: formData,
        });
        ...
        if (res.ok) {
          ...
          alert(errData.detail || "Error al extraer el archivo project.json del paquete ZIP.");
        }
      } catch (err) {
        console.error("Import ZIP error:", err);
        alert("Error de conexión al importar el archivo ZIP.");
      }
    }
    e.target.value = "";
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      infillDensity: store.infill,
      wallThickness: store.wallThickness,
      infillThickness: store.cellThickness,
      material: store.material,
      size: store.dimX * 10.0,
      showShell: true,
      cellSize: store.cellSize,
      orientation: store.orientation,
      resolution: store.resolution,
      layerHeight: store.layerHeight,
      printSpeed: store.printSpeed,
      viewportMode: "solid",
    ...
    try {
      const BACKEND_URL = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000";
      const res = await fetch(`${BACKEND_URL}/api/export_project`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        a.href = url;
        a.download = `cubo_proyecto_${store.material.toLowerCase()}_${store.pattern}.zip`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
      } else {
        alert("Error al generar el paquete del proyecto (.ZIP) en el servidor.");
      }
    } catch (err) {
      console.error("Export Project ZIP error:", err);
      alert("Error de conexión al exportar el paquete del proyecto.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleExportStl = async () => {
    ...
    try {
      await exportStl({
        ...
        infill: store.infill,
        wallThickness: store.wallThickness,
        cellThickness: store.cellThickness,
        material: store.material,
        dimX: store.dimX,
      });
    } catch (err) {
      console.error("Export STL error:", err);
      alert("Error de conexión al exportar archivo STL.");
    } finally {
      setExporting(false);
    }
  };
