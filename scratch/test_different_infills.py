...
def test_infills():
    ...
    # Test different infill densities
    for pct in [10.0, 20.0, 35.0, 50.0, 75.0]:
        ...
        try:
            ...
            if not mesh.is_watertight:
                mesh.fill_holes()
            print(f"Infill: {pct}% | Watertight: {mesh.is_watertight} | Vertices: {len(mesh.vertices)} | Faces: {len(mesh.faces)}")
        except Exception as e:
            print(f"Infill: {pct}% | Failed: {e}")
